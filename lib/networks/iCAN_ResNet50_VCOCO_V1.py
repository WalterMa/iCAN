...
class ResNet50():
    ...
    def build_network(self, is_training):
        ...
        res4_H = tf.reduce_mean(pool5_H, axis=[1, 2])
        Att_H = self.attention_pool_layer_H(head_phi, res4_H[:self.H_num, :], is_training, 'Att_H')
        Att_H = self.attention_norm_H(Att_H, 'Norm_Att_H')
        att_head_H = tf.multiply(head_g, Att_H)

        res4_O = tf.reduce_mean(pool5_O, axis=[1, 2])
        Att_O = self.attention_pool_layer_O(head_phi, res4_O, is_training, 'Att_O')
        ...
        self.visualize["attention_map_H"] = (Att_H - tf.reduce_min(Att_H[0, :, :, :])) / tf.reduce_max(
            (Att_H[0, :, :, :] - tf.reduce_min(Att_H[0, :, :, :])))
        self.visualize["attention_map_O"] = (Att_O - tf.reduce_min(Att_O[0, :, :, :])) / tf.reduce_max(
            (Att_O[0, :, :, :] - tf.reduce_min(Att_O[0, :, :, :])))
        return cls_prob_H, cls_prob_O, cls_prob_sp
